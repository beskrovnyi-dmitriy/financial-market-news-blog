...
function Article() {
    ...
    const articles = [{
        ...
        Here are 5 passive income ideas to consider in 2023.`, pic:''},
    {
        ...
        If you're worried about a debt hangover in 2023, you can set up a plan of attack. Here's how you can review your finances, figure out which debts to prioritize and choose a payoff strategy.`, pic: '' },
    {
        ...
        It's also not too late to reduce your taxable income for the year, and it could significantly trim your tax bill. So we asked some of our favorite financial experts for their best tips to make tax season a breeze. Here's what they had to say.`, pic: ''}];
    const [inputComment, setInputComment] = useState('');
    const [comments, setComments] = useState([]);

    function enterHandle(e,value){
        if(e.code==='Enter'){
            addComment(value);
        }
    }
    ...
    return (
        <>
            <div className="card mb-3" id={articleId}>
                <img src={articles[articleId-1].pic} className="card-img-top" alt="here suppose to be a picture" />
                <div className="card-body">
                    <h4 className="card-title">{articles[articleId-1].title}</h4>
                    <p className="card-text">{articles[articleId-1].text}</p>
                </div>
            </div>

            <div className="comments">
                <h4>{`Analyst's Comment`}</h4>
                <input autoFocus type="text" value={inputComment} onChange={e=>setInputComment(e.target.value)} onKeyDown={(e)=>enterHandle(e,inputComment)}/>
                <button onClick={()=>addComment(inputComment)}>add comment</button>
                <br/>
                <ul>{
                    comments.map(comment=>{
                    return (<div key={comment}><li>{comment}</li></div>)
                })}</ul>
            </div>
